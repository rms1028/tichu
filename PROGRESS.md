...
- **base APK rebuild (usesCleartextTraffic + dev deeplink)** — 로컬 dev 에서 서버 통신 복구 필요. `app.json` 의 `android.usesCleartextTraffic: true` 추가 + EAS build. 이게 없으면 visual-test scenario 11+ (GameScreen 진입 필요) 를 어떤 방식으로도 못 돌림.
- **GameScreen landscape 검증** — MatchmakingScreen 과 비슷한 slot-stretch 패턴이 GameScreen 에도 있는지 확인 필요. PlayerHand / OpponentHand / TableArea 레이아웃 점검.
- **visual-test scenario 11~15** — GameScreen 의 dealing / trick play / bomb window / dragon give / result. 위 두 가지가 선행 조건.
- **5단계** — 단일 실앱 + 3 결정적 봇 모드. 0~4단계 완료 이후 ROI 재평가 대상.
