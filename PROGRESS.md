...
- **1등 나간 이후 스몰 티츄 차단** (`game-engine.declareTichu` + `gameStore.onPhaseChanged`/`onPlayerFinished`) — 사용자 버그 리포트: "1등이 나간 상황인데 카드 1장도 안 낸 사람이 스몰 티츄를 누를 수 있음". 기존 규칙에 이 조건이 없었지만 실제 게임 규칙상 1등이 확정된 이후 스몰 티츄 선언은 **무조건 -100** 실패 확정 (1등을 빼앗을 방법 없음). 사용자 실수 방지 차원에서 차단. 수정: ① 서버 `declareTichu` 에 `type === 'small' && room.finishOrder.length > 0` → `someone_already_finished` 에러 반환 ② 클라이언트 `onPhaseChanged` 의 `canTichu` 계산에 `finishOrder.length === 0` 조건 추가 ③ 클라이언트 `onPlayerFinished` 에서 `canDeclareTichu: false` 명시 ④ 에러 번역 사전에 한글 메시지 추가. 131/131 테스트 통과.
- **커스텀 매치 게임 종료 후 대기실 복귀 + 상단 코인 배지 제거** (`AppRoot` + `CustomMatchScreen`) — 사용자 버그 리포트: "커스텀 매치에서 게임이 끝나면 모두 로비로 튕김, 대기화면으로 돌아가야 함". 근본 원인: 서버는 GAME_OVER 10초 후 `return_to_waiting` 을 emit 하며 `gameOver` 를 clear 하는데, `AppRoot` 의 result-screen 효과가 `!gameOver` 만 보고 무조건 `setScreen('lobby')` 호출. 수정: ① `matchMode === 'custom' && phase === 'WAITING_FOR_PLAYERS'` 면 `matchmaking` 으로 분기 ② Rematch 버튼도 커스텀이면 큐잉 없이 같은 방 대기실 복귀. 추가로 CustomMatchScreen 상단 코인 배지 (`coinsPill`/`coinDot`/`coinText` 스타일 포함) 제거.
- **Bug audit (9차-a 사전 점검)** — 사용자 요청 "또 다른 버그 있는지 전체 점검 (수정 X)". Explore subagent 결과를 직접 검증하니 주요 주장 2건 (one-two finish tichu bonus 이중 계산, bomb window 타이머 복원 누락) 은 오진. 실제로 확인된 이슈는 **무제한 턴 타이머 "0초" 표시** → 재검증 시 `remainingSec > 0` 가드가 있어 타이머가 숨겨지는 정상 동작이었음 (audit 자체 오진), **bombWindow 죽은 코드 + CLAUDE.md §6.6 드리프트**, `dog_lead_transfer` 1.5초 setTimeout 무가드, `resolveTrickWon` `.find()` 의미 불안정, `checkOneTwoFinish` 순서 미정의. 우선순위는 bombWindow 드리프트.
- **죽은 bombWindow 경로 + CLAUDE.md §6.6 드리프트 정리** (`gameStore`/`PlayerHand`/`GameScreen` + `socket-handlers` + CLAUDE.md) — `bomb-window.ts` 상단 NOTE 에 이미 "3초 딜레이 시스템 제거됨, 즉시 인터럽트로 처리" 로 명시되어 있었지만 부수 경로들이 남아 있었음: ① 클라이언트 `bombWindow` state + `onBombWindowStart`/`onBombWindowEnd` 액션 (어디서도 호출 안 됨), ② `PlayerHand.canSelectNormal` 의 `bombWindow !== null` 죽은 분기, ③ `GameScreen.tsx` 의 `bombWindow` 구독 + 타이머 일시정지 분기 (발화 조건 없음), ④ `socket-handlers.broadcastEvents` 의 `case 'bomb_window_start'` (호출 경로 없음), ⑤ `buildClientState.bombWindow` 스냅샷 필드 + `getAvailableBombs` import, ⑥ CLAUDE.md §6.6 전체 + §6.4/§6.5/§7.8/§7.9/§8.2/§8.4/§11.6/§12.3/§12.6 의 `BOMB_WINDOW` 참조. 전부 제거/수정. CLAUDE.md §6.6 은 "즉시 인터럽트 흐름" 으로 재작성하고 미래 작업자가 오해하지 않도록 "3초 윈도우 복구하려면..." 메모 추가. `bomb-window.ts` 의 deferred 함수들 (`startBombWindow`/`resolveBombWindow`/`afterBombWindowResolved`) 은 유닛 테스트에서 참조하므로 남겨둠. 131/131 테스트 통과, 양 패키지 typecheck clean.

### 9차 작업 (2026-04-15, CLAUDE.md 정리)

사용자 요청: CLAUDE.md 가 "Large CLAUDE.md will impact performance (41.3k chars > 40.0k)" 경고를 띄움. 문서 철학 ("코드가 단일 소스") 에 맞춰 축약하고 코드 드리프트 점검.

- `0c27296` **docs(claude.md): trim §11/§12 to stay under 40k char limit** — §11 족보 검증 알고리즘 나열 제거 → 함수별 파일 매핑 표 + 비자명한 소원/봉황 규칙만. §12 엣지 케이스 57 케이스 표 → `claude.md.bak` 과 테스트 파일 포인터 + 코드만 봐선 놓치기 쉬운 16개 규칙 bullet. 41.3k → 38.9k chars.
- `1d319e1` **docs(claude.md): fix drift + merge §7.5/§7.6 pipelines** — Explore subagent 로 CLAUDE.md 주장 vs 실제 코드 감사. 드리프트 2건 발견/수정: (1) §2 shared 테스트 파일 수 `7 → 8` (rng.test.ts 반영), (2) §4.3 visual regression baseline `01-splash ~ 04-custom-match (4개)` → 실제 `01-splash ~ 10-achievements (10개)`. §7.5 play_cards 11단계 + §7.6 submit_bomb 5단계 → 비자명한 검증 규칙 5 bullet 로 통합 (`game-engine.ts` / `socket-handlers.ts` 가 단일 소스).
- `895516f` **docs(claude.md): aggressive trim — 38.6k → 29.6k chars (-23%)** — 코드 재확인이 쉬운 섹션 대폭 축약: §2 디렉토리 ASCII 트리 (63줄) → 비자명한 위치 bullet 6줄, §3 워크스페이스별 반복 명령어 제거, §7.2 GameRoom TypeScript interface 복붙 (60줄) → 비자명한 필드 + RoomSettings 기본값만, §7.3 서버 모듈 맵 (§2 와 중복) 삭제, §7.7 declare_tichu 6단계 → 거부 조건 3 bullet, §8.4 공통 타입 복붙 → types.ts 포인터, §9.2 화면 13개 표 + §9.3 컴포넌트 19개 + §9.6 Utils 10개 → "ls 로 확인" + 비자명한 파일 6개만. 40k 한도 대비 ~10k 여유 확보.
