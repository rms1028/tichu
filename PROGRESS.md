...
이전 세션에서 "`pm clear` 가 Firebase Keystore 를 못 지우는 게 blocker" 라고 했는데 절반만 맞았다. 실제로는 `pm clear` 는 이미 6차 작업 `2258e4c` 에서 안정화됐고, 새 세션의 "스턱된 result screen" 은 앱 재설치로 근본 해결됨. 향후 비슷한 상황에서 먼저 `adb uninstall com.tichu.app && npm run android:dev` 시도.

### 남은 숙제

- **1단계-b** — `playLock` 영구 잠김 + `turnDuration === 0` truthy 체크 회귀 박제. `packages/app` 에 vitest 런너 부재가 블로커. 옵션: (a) vitest + jsdom 도입, (b) 로직만 shared 로 분리, (c) 보류.
- **2단계** — visual-test scenario 05+ 의 GameScreen 확장. Reanimated 카드 뷰가 adb tap 을 hit-test 통과 못하는 문제는 별도 작업 필요 (deeplink 테스트 모드 또는 accessibilityLabel).
- **5단계** — 단일 실앱 + 3 결정적 봇 모드. 0~4 단계 완료 이후 ROI 재평가 대상.
