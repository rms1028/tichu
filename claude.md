...
- React Native + Reanimated 뷰는 UIAutomator 에 노출되지 않음 → `uiautomator dump` 로 내부 버튼을 찾을 수 없다. PNG 에서 픽셀 좌표를 읽어 `adb shell input tap` 한다.
- 로고 glow / 파티클 애니메이션 때문에 `uiautomator dump` 가 "could not get idle state" 로 부분 덤프만 반환. 로비 이후 화면은 PNG 좌표가 유일한 신뢰 수단.
...
- 저장된 nickname (`useUserStore.nickname`) 이 있으면 login 스크린을 건너뛰어 `03-lobby` 시나리오 tap 이 엉뚱한 위치에 꽂힌다 → 깨끗한 baseline 은 `pm clear com.tichu.app` 후 재생성.
...
5. **폭탄 인터럽트:** 카드 제출 후 BOMB_WINDOW (3초). 상세는 6.5.
...
**해소 로직:** `pendingBombs` 비었으면 정상 진행. 있으면 최강 폭탄 선택, 나머지 핸드 복귀, 폭탄 적용 후 새 BOMB_WINDOW 시작. `windowId` 로 만료 콜백 보호.
...
3. 본인 중복 거부
4. [커스텀] 팀원 (seat+2%4) 선언 확인 → 있으면 거부 (`teammate_already_declared`)
5. 적용 + 브로드캐스트
```
...
### 11.6. getAvailableBombs

BOMB_WINDOW 용. 핸드에서 `currentTable` 보다 강한 모든 폭탄 반환.
...
| 2 | 봉황 리드 단독 | 값 1.5, 2 이상으로 제압 가능 |
| 3 | 봉황 싱글, 직전=참새(1) | 값=1.5 (가능) |
| 4 | 봉황 싱글, 직전=용 | 불가 (폭탄으로만) |
| 5 | 용 트릭, 양도 대상 나감 | 남은 상대에게 |
| 6 | 용 트릭, 상대 모두 나감 | 먼저 나간 상대 (finishOrder) 의 wonTricks 에 합산 |
